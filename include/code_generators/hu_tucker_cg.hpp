...
bool HuTuckerCG::genCodes (const std::vector<SymbolFreq>& symbol_freq_list,
    ...
    for (int i = 0; i < (int)symbol_list_.size(); i++) {
        Code code = lookup(i);
        symbol_code_list->push_back(std::make_pair(symbol_list[i], code));
    }
    return true;
}

double HuTuckerCG::getCompressionRate () {
    int64_t freq_sum = 0;
    ...
    for (int len = max_code_len; len > 0; len--) {
        ...
        for (int i = 0; i < (int)tmp_code_lens.size(); i++) {
            if (tmp_code_lens[i] == len)
                node_idx_list.push_back(i);
        }
        for (int i = 0; i < (int)node_idx_list.size(); i += 2) {
            idx1 = node_idx_list[i];
            idx2 = node_idx_list[i + 1];
...
void HuTuckerCG::mergeNodes(const int idx1, const int idx2) {
    ...
    Node* new_node = new Node(left_node->left_idx, right_node->right_idx,
