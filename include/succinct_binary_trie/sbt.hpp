...
bool SBT::lookup(const std::string& in_key, int& key_bit_pos, int* out_idx) {
	...
	if (cur_char & (1 << (7 - offset)))
	    pos += 1;
	moveToLeftChild(pos);
	if (pos >= trie_->numBits())
	    return false;
	if (isLeaf(pos)) {
	    key_bit_pos++;
...
bool SBT::isLeaf(const int pos) {
    return !trie_->readBit(pos) && !trie_->readBit(pos + 1);
}
...
int SBT::memory() {
    return (sizeof(SBT) + trie_->size() + num_nodes_ * sizeof(int));
}
