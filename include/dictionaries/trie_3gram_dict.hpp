...
class Trie3GramDict : public Dictionary {
 ...
 private:
  void buildTrie(const std::vector<SymbolCode> &symbol_code_list, std::vector<TrieNode> &level_1,
                 std::vector<TrieNode> &level_2, std::vector<IntervalCode> &leafs);
...
void Trie3GramDict::buildTrie(const std::vector<SymbolCode> &symbol_code_list, std::vector<TrieNode> &level_1,
                              std::vector<TrieNode> &level_2, std::vector<IntervalCode> &leafs) {
...
void Trie3GramDict::copyNodesToArray(const std::vector<TrieNode> &level_1, const std::vector<TrieNode> &level_2,
                                     const std::vector<IntervalCode> &leafs) {
...
Code Trie3GramDict::lookup(const char *symbol, const int symbol_len, int &prefix_len) const {
  int char_idx_0 = (uint8_t)symbol[0];
  int char_idx_1 = -1;
  if (symbol_len > 1) char_idx_1 = (uint8_t)symbol[1];
  int char_idx_2 = -1;
