...
static const std::string file_x_email_dict_size = output_dir + cpr_latency_subdir + "x_email_dict_size.csv";
std::ofstream output_x_email_dict_size;
static const std::string file_cpr_email_dict_size = output_dir + cpr_latency_subdir + "cpr_email_dict_size.csv";
std::ofstream output_cpr_email_dict_size;
static const std::string file_lat_email_dict_size = output_dir + cpr_latency_subdir + "lat_email_dict_size.csv";
std::ofstream output_lat_email_dict_size;
static const std::string file_mem_email_dict_size = output_dir + cpr_latency_subdir + "mem_email_dict_size.csv";
std::ofstream output_mem_email_dict_size;

static const std::string file_x_wiki_dict_size = output_dir + cpr_latency_subdir + "x_wiki_dict_size.csv";
std::ofstream output_x_wiki_dict_size;
static const std::string file_cpr_wiki_dict_size = output_dir + cpr_latency_subdir + "cpr_wiki_dict_size.csv";
std::ofstream output_cpr_wiki_dict_size;
static const std::string file_lat_wiki_dict_size = output_dir + cpr_latency_subdir + "lat_wiki_dict_size.csv";
std::ofstream output_lat_wiki_dict_size;
static const std::string file_mem_wiki_dict_size = output_dir + cpr_latency_subdir + "mem_wiki_dict_size.csv";
std::ofstream output_mem_wiki_dict_size;

static const std::string file_x_url_dict_size = output_dir + cpr_latency_subdir + "x_url_dict_size.csv";
std::ofstream output_x_url_dict_size;
static const std::string file_cpr_url_dict_size = output_dir + cpr_latency_subdir + "cpr_url_dict_size.csv";
std::ofstream output_cpr_url_dict_size;
static const std::string file_lat_url_dict_size = output_dir + cpr_latency_subdir + "lat_url_dict_size.csv";
std::ofstream output_lat_url_dict_size;
static const std::string file_mem_url_dict_size = output_dir + cpr_latency_subdir + "mem_url_dict_size.csv";
std::ofstream output_mem_url_dict_size;

static const std::string file_x_ts_dict_size = output_dir + cpr_latency_subdir + "x_ts_dict_size.csv";
std::ofstream output_x_ts_dict_size;
static const std::string file_cpr_ts_dict_size = output_dir + cpr_latency_subdir + "cpr_ts_dict_size.csv";
std::ofstream output_cpr_ts_dict_size;
static const std::string file_lat_ts_dict_size = output_dir + cpr_latency_subdir + "lat_ts_dict_size.csv";
std::ofstream output_lat_ts_dict_size;
static const std::string file_mem_ts_dict_size = output_dir + cpr_latency_subdir + "mem_ts_dict_size.csv";
...
static const std::string file_x_email_ht = output_dir + ht_vs_dc_subdir + "x_email_ht.csv";
std::ofstream output_x_email_ht;
static const std::string file_cpr_email_ht = output_dir + ht_vs_dc_subdir + "cpr_email_ht.csv";
std::ofstream output_cpr_email_ht;
static const std::string file_lat_email_ht = output_dir + ht_vs_dc_subdir + "lat_email_ht.csv";
std::ofstream output_lat_email_ht;
static const std::string file_bt_email_ht = output_dir + ht_vs_dc_subdir + "bt_email_ht.csv";
std::ofstream output_bt_email_ht;

static const std::string file_x_email_dc = output_dir + ht_vs_dc_subdir + "x_email_dc.csv";
std::ofstream output_x_email_dc;
static const std::string file_cpr_email_dc = output_dir + ht_vs_dc_subdir + "cpr_email_dc.csv";
std::ofstream output_cpr_email_dc;
static const std::string file_lat_email_dc = output_dir + ht_vs_dc_subdir + "lat_email_dc.csv";
std::ofstream output_lat_email_dc;
static const std::string file_bt_email_dc = output_dir + ht_vs_dc_subdir + "bt_email_dc.csv";
...
static const std::string file_batch_lat = output_dir + batch_subdir + "batch_lat.csv";
...
void getSampleKeys(const double sample_percent, std::vector<std::string> &sample_keys,
                   const std::vector<std::string> &all_keys, int64_t &enc_src_len) {
...
void exec(const int expt_id, const int wkld_id, const int encoder_type, const int64_t dict_size_id,
          const double sample_percent, const double enc_percent, std::vector<std::string> &keys_shuffle,
  ...
  double dict_size = 0;
  double mem = 0;
  exec_helper(encoder_type, W, input_dict_size, sample_keys, enc_src_keys, enc_src_len, encode_method, batch_size, bt,
              tput, lat, cpr, dict_size, mem);

  if (expt_id < 0 || expt_id > 9) std::cout << "ERROR: INVALID EXPT ID!" << std::endl;

  if (wkld_id < 0 || wkld_id > 3) std::cout << "ERROR: INVALID WKLD ID!" << std::endl;
  ...
  if (expt_id == 0) {
    ...
    for (int bs = 0; bs < 7; bs++) {
      ...
      for (int encoder_type = 1; encoder_type < 5; encoder_type++) {
        ...
        exec(expt_id, kEmail, encoder_type, ds, sample_percent, enc_src_percent, emails_shuffle, total_len_email, 2,
             batch_size);
      }
    }
    runBatch = false;
    ...
    exec_helper(encoder_type, W, input_dict_size, sample2_keys, emails1_shuffle, total_len_email1, encode_method,
                batch_size, bt, tput, lat, cpr, dict_size, mem);
    std::cout << "---------------Dataset 2, Dictionary 1-----------------" << std::endl;
    exec_helper(encoder_type, W, input_dict_size, sample1_keys, emails2_shuffle, total_len_email2, encode_method,
                batch_size, bt, tput, lat, cpr, dict_size, mem);
  }
  return 0;
}
