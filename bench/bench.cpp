...
int main(int argc, char *argv[]) {
    ...
    //exec(0, 1000, emails, emails_shuffle, total_len_email);
    ...
    //exec(0, 1000, wikis, wikis_shuffle, total_len_wiki);

    std::vector<std::string> urls;
    std::vector<std::string> urls_shuffle;
    int64_t total_len_url = loadKeys(file_url, urls, urls_shuffle);
    //exec(0, 1000, urls, urls_shuffle, total_len_url);

    exec(1, 65536, emails, emails_shuffle, total_len_email);
    exec(1, 65536, wikis, wikis_shuffle, total_len_wiki);
    exec(1, 65536, urls, urls_shuffle, total_len_url);
