...
int main(int argc, char *argv[]) {
  ...
  if (is_compressed_int > 0) is_compressed = true;
  int encoder_type = (int)atoi(argv[6]);
  int64_t dict_size_limit = (int64_t)atoi(argv[7]);

  // check args ====================================================
  if (filter_type.compare(std::string("SuRF")) != 0 && filter_type.compare(std::string("SuRFHash")) != 0 &&
  ...
  if (key_type.compare(std::string("email")) != 0 && key_type.compare(std::string("wiki")) != 0 &&
  ...
  double fp_rate = 0;
  if (false_positives > 0) fp_rate = false_positives / (true_negatives + false_positives + 0.0);
  std::cout << kGreen << "False Positive Rate = " << kNoColor << fp_rate << "\n";

  double mem = filter->getMemoryUsage() + 0.0;
  if (encoder != nullptr) mem += encoder->memoryUse();
  mem /= 1000000;  // MB
