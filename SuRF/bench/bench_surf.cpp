...
void selectKeysToInsert(const unsigned percent, std::vector<std::string> &insert_keys, std::vector<std::string> &keys) {
  ...
  for (uint64_t i = 0; i < num_insert_keys; i++) insert_keys.push_back(keys[i]);
...
int64_t computePointTruePositives(const std::vector<std::string> insert_keys, const std::vector<std::string> txn_keys) {
  std::map<std::string, bool> ht;
  for (int i = 0; i < (int)insert_keys.size(); i++) ht[insert_keys[i]] = true;
...
int64_t computeRangeTruePositives(const std::vector<std::string> insert_keys, const std::vector<std::string> txn_keys,
...
void loadWorkload(int wkld_id, int64_t &point_tp, int64_t &range_tp, std::vector<std::string> &load_keys,
                  std::vector<std::string> &insert_keys, std::vector<std::string> &insert_keys_shuffle,
                  std::vector<std::string> &insert_keys_sample, std::vector<std::string> &txn_keys,
  ...
  for (int i = 0; i < (int)insert_keys_shuffle.size(); i += int(100 / kSamplePercent)) {
...
void exec(const int expt_id, const int wkld_id, const bool is_point, const int filter_type, const uint32_t suffix_len,
          const bool is_compressed, const int encoder_type, const int64_t dict_size_id,
  ...
  if (is_point) {  // point query
    if (is_compressed) {
      ...
      for (int i = 0; i < (int)txn_keys.size(); i++) positives += (int)filter->lookupKey(txn_keys[i]);
    }
  } else {  // range query
    if (is_compressed) {
      for (int i = 0; i < (int)txn_keys.size(); i++) {
...
void exec_group(const int expt_id, const int filter_type, const uint32_t suffix_len, const bool is_point, int &expt_num,
                const int total_num_expt, const int64_t email_point_tp, const int64_t email_range_tp,
                const int64_t wiki_point_tp, const int64_t wiki_range_tp, const int64_t url_point_tp,
                const int64_t url_range_tp, const std::vector<std::string> &insert_emails,
                const std::vector<std::string> &insert_emails_sample, const std::vector<std::string> &txn_emails,
                const std::vector<std::string> &upper_bound_emails, const std::vector<std::string> &insert_wikis,
                const std::vector<std::string> &insert_wikis_sample, const std::vector<std::string> &txn_wikis,
                const std::vector<std::string> &upper_bound_wikis, const std::vector<std::string> &insert_urls,
                const std::vector<std::string> &insert_urls_sample, const std::vector<std::string> &txn_urls,
...
int main(int argc, char *argv[]) {
  ...
  if (expt_id == 0) {
    ...
    exec_group(expt_id, 2, 8, is_point, expt_num, total_num_expt, email_point_tp, email_range_tp, wiki_point_tp,
               wiki_range_tp, url_point_tp, url_range_tp, insert_emails, insert_emails_sample, txn_emails,
               upper_bound_emails, insert_wikis, insert_wikis_sample, txn_wikis, upper_bound_wikis, insert_urls,
               insert_urls_sample, txn_urls, upper_bound_urls);
