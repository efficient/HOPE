...
/*
        TEST_F(HeuristicEnCoderTest, wordTest) {
                ...
                for (int i = 0; i < (int)words.size() - 1; i++) {
                    ...
                    total_enc_len += len;
                    std::string str1 = std::string((const char*)buffer,
   getByteLen(len)); len = encoder->encode(words[i + 1], buffer); std::string
   str2 = std::string((const char*)buffer, getByteLen(len)); int cmp =
   str1.compare(str2);
                ...
                delete[] buffer;
                delete encoder;
                std::cout << "cpr = " << ((total_len + 0.0) / total_enc_len) <<
   std::endl;
        }
        ...
        TEST_F (HeuristicEnCoderTest, emailTest) {
            ...
            for (int i = 0; i < (int)emails.size() - 1; i++) {
                ...
                total_enc_len += len;
                std::string str1 = std::string((const char*)buffer,
   getByteLen(len)); len = encoder->encode(emails[i + 1], buffer); std::string
   str2 = std::string((const char*)buffer, getByteLen(len)); int cmp =
   str1.compare(str2); if (cmp >= 0) { int len1 = encoder->encode(emails[i],
   buffer); std::cout << emails[i] << "\t" << len1 << std::endl; print(str1);
