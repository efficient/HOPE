...
TEST_F(HeuristicSSTest, getInterval) {
  /*            HeuristicSS ss;
              std::vector<SymbolFreq> v;
              v.push_back(std::make_pair("aba",300));
              v.push_back(std::make_pair("abb",100));
              v.push_back(std::make_pair("ac",300));
              ss.getSkewInterval(v);
              for (auto iter = ss.intervals_.begin(); iter !=
     ss.intervals_.end(); iter++) { if (iter->first[0] == 'a') std::cout << "["
     << iter->first << ", " << iter->second << ")" << std::endl;
              ...
              v.clear();
              v.push_back(std::make_pair("aba",300));
              v.push_back(std::make_pair("abb",300));
              v.push_back(std::make_pair("ac",200));
              ss1.getSkewInterval(v);
              for (auto iter = ss1.intervals_.begin(); iter !=
     ss1.intervals_.end(); iter++) { if (iter->first[0] == 'a') std::cout << "["
     << iter->first << ", " << iter->second << ")" << std::endl;
              }
  */
}
...
TEST_F(HeuristicSSTest, fillGapSmall) {
  //            HeuristicSS ss;
  //            ss.fillGap(std::string(1, char(230)), std::string(1,
  //            char(235))); for (auto iter = ss.intervals_.begin(); iter !=
  //            ss.intervals_.end(); iter++) {
  ...
  //            std::cout << "------------------------" << std::endl;
  //            HeuristicSS ss1;
  //            ss1.fillGap(std::string(1, char(125)), std::string(1,
  //            char(200))); for (auto iter = ss1.intervals_.begin(); iter !=
  //            ss1.intervals_.end(); iter++) { 	std::cout << "[" <<
  //            iter->first
  ...
  //            HeuristicSS ss2;
  //            ss2.fillGap("beq", "ber");
  //            for (auto iter = ss2.intervals_.begin(); iter !=
  //            ss2.intervals_.end(); iter++) {
  //             	std::cout << "[" << iter->first << ", " << iter->second
  //             << ")" << std::endl;
  //            }
}
...
TEST_F(HeuristicSSTest, getPrevString) {
  ...
  // ASSERT_TRUE(ss.getPrevString("abc" + std::string(1,char(0))).compare("abc")
  // == 0);
}
