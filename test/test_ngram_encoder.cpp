...
TEST_F (NGramEncoderTest, word3PairTest) {
    ...
    for (int i = 0; i < (int)words.size() - 1; i++) {
        total_len += (words[i].length() * 8);
        int l_len = 0, r_len = 0;
        encoder->encodePair(words[i], words[i + 1], l_buffer, r_buffer, l_len,
r_len); total_enc_len += l_len; std::string str1 = std::string((const
char*)l_buffer, getByteLen(l_len)); std::string str2 = std::string((const
char*)r_buffer, getByteLen(r_len)); int cmp = str1.compare(str2);
        ASSERT_TRUE(cmp < 0);
    }
    std::cout << "cpr = " << ((total_len + 0.0) / total_enc_len) << std::endl;
}
...
/*TEST_F (NGramEncoderTest, email4PairTest) {
    ...
    for (int i = 0; i < (int)emails.size() - 1; i++) {
        ...
        int l_len = 0, r_len = 0;
        encoder->encodePair(emails[i], emails[i + 1], l_buffer, r_buffer, l_len,
r_len); total_enc_len += l_len; std::string str1 = std::string((const
char*)l_buffer, getByteLen(l_len)); std::string str2 = std::string((const
char*)r_buffer, getByteLen(r_len)); int cmp = str1.compare(str2);
        ASSERT_TRUE(cmp < 0);
